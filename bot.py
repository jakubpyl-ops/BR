...
- Propozycje: ustaw kanał (/konfiguracja kanal Propozycje). Każda wiadomość napisana na tym
  kanale (tekst i/lub zdjęcie) zamienia się automatycznie w kartę propozycji z głosowaniem
  na przyciskach ✅ TAK / ❌ NIE (z wynikami i procentami), a pod spodem bot sam zakłada
  wątek do dyskusji.
...
- Błędy, ankiety i konkursy da się edytować i usuwać komendami staffu/adminów - patrz /pomoc.
"""
...
DEFAULT_CONFIG = {
    ...
    "liczniki": {
        "blad": 0, "ankieta": 0, "ticket": 0, "faq": 0, "konkurs": 0,
    },
...
class ZgloszeniePanel(discord.ui.LayoutView):
    def __init__(self, typ: str):
        ...
        opis = ("Znalazłeś błąd w grze lub na serwerze? Kliknij przycisk niżej i opisz co się dzieje - "
...
def wyniki_glosowania_tekst(message_id: int) -> str:
...
class PropozycjaGlosowanieView(discord.ui.View):
    """Rejestrowany raz, globalnie - przyciski mają stałe custom_id (nie zależą od
    konkretnej propozycji), więc obsługują głosowanie pod KAŻDĄ kartą naraz."""
    ...
    async def _glosuj(self, interaction: discord.Interaction, wybor: str):
        ...
        embed.set_field_at(2, name="📊 Wyniki", value=wyniki_glosowania_tekst(interaction.message.id), inline=False)
        await interaction.response.edit_message(embed=embed)

    @discord.ui.button(label="TAK", emoji="✅", style=discord.ButtonStyle.success, custom_id="igrzyskamc:propozycja:tak")
    async def tak(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._glosuj(interaction, "tak")

    @discord.ui.button(label="NIE", emoji="❌", style=discord.ButtonStyle.danger, custom_id="igrzyskamc:propozycja:nie")
...
async def obsluz_wiadomosc_propozycji(message: discord.Message):
    """Każda wiadomość na kanale propozycji (tekst i/lub zdjęcie) zamienia się w kartę
    z głosowaniem ✅ TAK / ❌ NIE - wyniki i procenty liczone są na bieżąco, a pod spodem
    bot zakłada wątek, żeby dyskusja nie zaśmiecała głównego kanału."""
    ...
    nazwa = CONFIG.get("nazwa_serwera", "Bot")
    rok = datetime.datetime.now().year
    embed = discord.Embed(title=f"{message.guild.name} x Propozycja", color=get_kolor("propozycje"))
    embed.add_field(name="👤 Od", value=autor.mention, inline=False)
    embed.add_field(name="💡 Propozycja", value=tresc[:1024] if tresc else "*(brak treści — dodano zdjęcie)*", inline=False)
    embed.add_field(name="📊 Wyniki", value="✅ 0 (0%)   ❌ 0 (0%)", inline=False)
    if obrazek:
        embed.set_image(url=obrazek)
    embed.set_footer(text=f"© {rok} {nazwa} x Propozycje")

    wiadomosc = await message.channel.send(embed=embed, view=PropozycjaGlosowanieView())
    try:
        await wiadomosc.create_thread(name=f"Propozycja | {autor.display_name}"[:100], auto_archive_duration=1440)
    except discord.HTTPException:
        pass
...
POMOC_KATEGORIE = {
    ...
    "propozycje": {
        "etykieta": "💡 Propozycje",
        "tresc": (
            "Ustaw kanał propozycji w `/konfiguracja kanal` (typ: Propozycje) — każda wiadomość "
            "napisana tam (tekst i/lub zdjęcie) sama zamieni się w kartę propozycji z przyciskami "
            "**✅ TAK** / **❌ NIE**. Wyniki głosowania (liczba głosów i procenty) aktualizują się "
            "na bieżąco, a pod każdą kartą bot automatycznie zakłada wątek do dyskusji.\n\n"
...
async def on_ready():
    ...
    bot.add_view(PropozycjaGlosowanieView())
